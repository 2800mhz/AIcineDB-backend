...
class DatabaseOperations:
    ...
    async def _create_narrative(self, film_id: int, narrative: Dict):
        ...
        # JSONB columns - keep as JSON string
        themes_raw = narrative.get('themes', [])
        themes = json.dumps(themes_raw) if isinstance(themes_raw, (list, dict)) else (themes_raw if isinstance(themes_raw, str) else '[]')
        
        story_beats_raw = narrative.get('story_beats', [])
        story_beats = json.dumps(story_beats_raw) if isinstance(story_beats_raw, (list, dict)) else (story_beats_raw if isinstance(story_beats_raw, str) else '[]')
        
        act_structure_raw = narrative.get('act_structure') or narrative.get('structure', {})
        act_structure = json.dumps(act_structure_raw) if isinstance(act_structure_raw, (list, dict)) else (act_structure_raw if isinstance(act_structure_raw, str) else '{}')
        
        # TEXT[] columns - convert to list
        genre_raw = narrative.get('genre', [])
        if isinstance(genre_raw, str):
            genre = [genre_raw]
        elif isinstance(genre_raw, list):
            genre = [str(x) for x in genre_raw]
        else:
            genre = []
        
        tone_raw = narrative.get('tone', [])
        if isinstance(tone_raw, str):
            tone = [tone_raw]
        elif isinstance(tone_raw, list):
            tone = [str(x) for x in tone_raw]
        else:
            tone = []
        
        # FLOAT[] columns - convert to float list
        emotional_arc_raw = narrative.get('emotional_arc', [])
        emotional_arc = []
        if isinstance(emotional_arc_raw, str):
            try:
                emotional_arc = [float(x) for x in json.loads(emotional_arc_raw)]
            except:
                emotional_arc = []
        elif isinstance(emotional_arc_raw, list):
            for val in emotional_arc_raw:
                try:
                    emotional_arc.append(float(val))
                except:
                    continue
        
        # Build the query WITHOUT type casting in placeholders
        query = """
        ...
        try:
            ...
            logger.error(f"   Data types:")
            logger.error(f"     themes: {type(themes)} = {themes[:100] if themes else None}")
            logger.error(f"     genre: {type(genre)} = {genre}")
            logger.error(f"     tone: {type(tone)} = {tone}")
            logger.error(f"     emotional_arc: {type(emotional_arc)} = {emotional_arc[:5] if emotional_arc else []}")
            logger.error(f"     story_beats: {type(story_beats)} = {story_beats[:100] if story_beats else None}")
            logger.error(f"     act_structure: {type(act_structure)} = {act_structure[:100] if act_structure else None}")
            
            # Try to continue without narrative (non-critical data)
            logger.warning("⚠️ Skipping narrative save to allow analysis to complete")
            return
